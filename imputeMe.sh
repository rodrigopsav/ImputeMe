...
##### SET PARAMETERS imputeMe_run.sh #####
...
##### Run imputeMeLocal or imputeMeSlurm #####
export IMPUTEME_DIR=$(dirname $(readlink -f $0))
...
#
if [[ ! -f "$CONFIGSLURM" ]]; then
   source $IMPUTEME_DIR/program/imputeMeLocalRun.sh
else
   for par in $(cat $CONFIGSLURM); do export $par; done
   source $IMPUTEME_DIR/program/imputeMeSlurmRun.sh
fi
