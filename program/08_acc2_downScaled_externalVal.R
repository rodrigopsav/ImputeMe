...
#---------- G matrix of observed data ----------#
print("SVD G matrix of observed data")
# GRM gcta
G_obs <- ReadGRMBin(paste0("obsGRM_", pos))

# PCA gcta
eigenval_obs <- fread(paste0("obsGRM_", pos, ".eigenval"))
eigenval_obs <- as.data.frame(eigenval_obs)
variances_obs <- (eigenval_obs$V1^2) / sum(eigenval_obs$V1^2)

eigenvec_obs <- fread(paste0("obsGRM_", pos, ".eigenvec"))
eigenvec_obs <- as.data.frame(eigenvec_obs)
scores_obs <- eigenvec_obs[,c(3:4, 2)]
colnames(scores_obs) <- c("pc1", "pc2", "id")
scores_obs$pop <- "Observed"


#---------- G matrix of imputed data ----------#
print("SVD G matrix of imputed data")
# GRM gcta
G_pred <- ReadGRMBin(paste0("predGRM_", pos))

# PCA gcta
eigenval_pred <- fread(paste0("predGRM_", pos, ".eigenval"))
eigenval_pred <- as.data.frame(eigenval_pred)
variances_pred <- (eigenval_pred$V1^2) / sum(eigenval_pred$V1^2)

eigenvec_pred <- fread(paste0("predGRM_", pos, ".eigenvec"))
eigenvec_pred <- as.data.frame(eigenvec_pred)
scores_pred <- eigenvec_pred[ ,c(3:4, 2)]
colnames(scores_pred) <- c("pc1", "pc2", "id")
scores_pred$pop <- "Imputed"


#---------- Concatenate scores_pred Observed and Imputed G matrix ----------#
pca_scores <- rbind(scores_obs, scores_pred)
...
p4 <- ggplot(pca_scores, aes_string(x=colnames(pca_scores)[pc1], y=colnames(pca_scores)[pc2], col=colnames(pca_scores)[which(colnames(pca_scores) == "pop")])) + 
  ...
  theme(#panel.grid.major.x = element_blank(),
    ...
    legend.position = "right") +
  xlab(paste("PC",pc1, " (",variances_obs[pc1],"%)",sep="")) + 
  ylab(paste("PC",pc2, " (",variances_obs[pc2],"%)",sep=""))
