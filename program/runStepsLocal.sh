...
(
echo "#@#############################################################"
echo "#@                    ImputeME: $OUTPUT_NAME"
echo
D1=$(date "+%D    %T")
echo "#@ Date and Time: $D1"
echo
) > $OUTPUT_DIR/imputeMe_"$OUTPUT_NAME"/log/log_"$OUTPUT_NAME".txt
...
(
   ...
   echo "Running chromosome $FileExtension" >> $OUTPUT_DIR/imputeMe_"$OUTPUT_NAME"/log/log_"$OUTPUT_NAME".txt
   wait
...
#while ! grep -q "took" $OUTPUT_DIR/imputeMe_"$OUTPUT_NAME"/log/log_"$OUTPUT_NAME".txt; do
#   sleep 5m
#   echo >> $OUTPUT_DIR/imputeMe_"$OUTPUT_NAME"/log/log_"$OUTPUT_NAME".txt
#   echo "ImputeMe still running ... $(date +'%r')  -  $(date +'%m/%d/%Y')" >> $OUTPUT_DIR/imputeMe_"$OUTPUT_NAME"/log/log_"$OUTPUT_NAME".txt
#done
#wait
