...
echo
echo "#@############################################################"
echo "#@                 CHECK ImputeMe PARAMETERS"
#echo
...
if [[ -z "$GENETIC_MAP" || "$GENETIC_MAP" == "1cMperMb" ]]; then
   ...
   if [[ "PHASING_PROGRAM" == "eagle" ]]; then
      export GENETIC_MAP=$IMPUTEME_DIR/program/genetic_map_1cMperMb.txt
   else
...
if [[ -z "$MAF" ]]; then
   ...
   echo "ImputeMe will skip maf for filtering"
   export MAF=0
   COUNT_WARNING=$((COUNT_WARNING + 1))
   echo
else
...
if [[ -z "$MISSING" ]]; then
   ...
   echo "ImputeMe will skip missingness snps for filtering"
   export MISSING=0
   COUNT_WARNING=$((COUNT_WARNING + 1))
   echo
else
...
if [[ -z "$MEM" ]]; then
   ...
   echo "ImputeMe will consider MEM=16"
   export MEM=16
   COUNT_WARNING=$((COUNT_WARNING + 1))
   echo
else
...
if ! command -v vcftools &> /dev/null; then
   ...
   echo "Please install vcftools using install_imputeme_dependencies.sh"
   COUNT_ERROR=$((COUNT_ERROR + 1))
   echo
fi
wait
...
if ! command -v bcftools &> /dev/null; then
   ...
   echo "Please install bcftools using install_imputeme_dependencies.sh"
   COUNT_ERROR=$((COUNT_ERROR + 1))
   echo
fi
wait
...
if ! command -v $CONDA/bin/eagle &> /dev/null; then
   ...
   echo "Please install eagle using install_imputeme_dependencies.sh"
   COUNT_ERROR=$((COUNT_ERROR + 1))
   echo
fi
wait
...
if ! command -v java -jar $CONDA/bin/beagle &> /dev/null; then
   ...
   echo "Please install beagle using install_imputeme_dependencies.sh"
   COUNT_ERROR=$((COUNT_ERROR + 1))
   echo
fi
wait
...
if [[ "$COUNT_ERROR" -gt "0" ]]; then
   echo
   echo "Solve all the ERRORS before run ImputeMe again "
   echo "Don't worry about WARNINGS: ImputeMe will use default parameters "
