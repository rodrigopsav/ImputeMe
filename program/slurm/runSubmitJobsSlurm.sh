...
for REGION in $SELECT_CHROM; do
   ...
   --output $OUTPUT_DIR/imputeMe_"$OUTPUT_NAME"/logSlurm/"$FileExtension".txt \
   --export=CONDA=$CONDA,IMPUTEME_DIR=$IMPUTEME_DIR,REF_GENOME=$REF_GENOME,REF_PANEL=$REF_PANEL,VAL_PANEL_DIR=$VAL_PANEL_DIR,VAL_PANEL=$VAL_PANEL,GENO_PANEL=$GENO_PANEL,GENETIC_MAP=$GENETIC_MAP,OUTPUT_DIR=$OUTPUT_DIR,OUTPUT_NAME=$OUTPUT_NAME,PHASING_PROGRAM=$PHASING_PROGRAM,IMPUTE_PROGRAM=$IMPUTE_PROGRAM,CHROM_SET=$CHROM_SET,INCLUDE_CHROM_X=$INCLUDE_CHROM_X,MAF=$MAF,MAF=$MAF,MISSING=$MISSING,SWITCH=$SWITCH,DOWN_SCALING=$DOWN_SCALING,FileExtension=$FileExtension,REGION=$REGION,CHR=$CHR,BPSTART=$BPSTART,BPEND=$BPEND,DOWN_SCALED=$DOWN_SCALED,FILTER=$FILTER,PHASE=$PHASE,IMPUTE=$IMPUTE,ACC=$ACC \
   $IMPUTEME_DIR/program/slurm/runStepsSlurm.sb)
   echo $JOBID1 >> $OUTPUT_DIR/imputeMe_"$OUTPUT_NAME"/logSlurm/jobid1.txt
done


WAIT1=$(cat $OUTPUT_DIR/imputeMe_"$OUTPUT_NAME"/logSlurm/jobid1.txt | awk '{print $4}' | tr "\n" ":" | sed 's/.$//')
wait

##### IMPUTATION ACCURACY
sbatch --dependency=afterok:$WAIT1 \
--job-name="${ANALYSIS_ID}"_"$OUTPUT_NAME"_accR --time=$ACC_TIME --cpus-per-task=$ACC_CPU --mem=$ACC_MEM \
--output=$OUTPUT_DIR/imputeMe_"$OUTPUT_NAME"/logSlurm/accR.txt \
--export=CONDA=$CONDA,IMPUTEME_DIR=$IMPUTEME_DIR,REF_GENOME=$REF_GENOME,VAL_PANEL_DIR=$VAL_PANEL_DIR,GENO_PANEL=$GENO_PANEL,DOWN_SCALING=$DOWN_SCALING,PHASE=$PHASE,ACC=$ACC,OUTPUT_DIR=$OUTPUT_DIR,OUTPUT_NAME=$OUTPUT_NAME,PARAMETERS=$PARAMETERS \
$IMPUTEME_DIR/program/slurm/run_accR_Slurm.sb
wait
