...
# PCA gcta
eigenval <- fread(paste0("genoGRM_", pos, ".eigenval"))
eigenval <- as.data.frame(eigenval_obs)
variances <- (eigenval_obs$V1^2) / sum(eigenval_obs$V1^2)

eigenvec <- fread(paste0("genoGRM_", pos, ".eigenvec"))
eigenvec <- as.data.frame(eigenvec)
pca_scores <- eigenvec[,c(3:4, 2)]
colnames(pca_scores) <- c("pc1", "pc2", "id")
...
p3 <- ggplot(pca_scores, aes_string(x=colnames(pca_scores)[pc1], y=colnames(pca_scores)[pc2])) + 
