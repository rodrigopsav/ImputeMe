...
set -m
nohup $IMPUTEME_DIR/program/runStepsLocal.sh >/dev/null 2>&1 &
echo $! > $OUTPUT_DIR/imputeMe_"$OUTPUT_NAME"/log/pid.txt
echo "Running ImputeMe"
echo "ANALYSIS NAME: $OUTPUT_NAME"
echo "General log: tail -f -n +1 $OUTPUT_DIR/imputeMe_${OUTPUT_NAME}/log/log_${OUTPUT_NAME}.txt"
echo "Check the log files per chromosome in $OUTPUT_DIR/imputeMe_${OUTPUT_NAME}/log"
echo "To open the log file of a chromosome, type on terminal: tail -f -n +1 $OUTPUT_DIR/imputeMe_${OUTPUT_NAME}/log/${OUTPUT_NAME}_CHR.txt"
echo "Just replace CHR by the chromosome name"
echo "To kill this ImputeMe analysis, run the command line: kill -- -$(cat $OUTPUT_DIR/imputeMe_"$OUTPUT_NAME"/log/pid.txt)"
