...
gcta64 \
   --grm-bin $ACC/012/obsGRM_"$FileExtension" \
   --pca 2 \
   --threads $THREADS \
   --out $ACC/012/obsGRM_"$FileExtension" > /dev/null 2>&1
wait
...
java -jar $IMPUTEME_DIR/program/LDkit.jar \
...
rm $ACC/012/obsGRM_"$FileExtension".log > /dev/null 2>&1 
...
##### IMPUTED POPULATION #####
### MAP
cat $IMPUTE/imputed_"$FileExtension".info | awk '{if ($8 == "Genotyped") print $1"\t"$8}' | awk -F":" '{print $1"\t"$2}' | tail -n +2 > $ACC/posGenotyped_"$FileExtension".txt
cat $IMPUTE/imputed_"$FileExtension".info | awk '{if ($8 == "Imputed") print $1"\t"$8}' | awk -F":" '{print $1"\t"$2}' > $ACC/posImputed_"$FileExtension".txt
...
### EXTRACT R2 and MAF
cat $IMPUTE/imputed_"$FileExtension".info | awk -F"\t" '{print $1"\t"$8"\t"$7"\t"$5}' | awk -F":" '{print $1"\t"$2}' | tail -n +2 > $ACC/r2_maf/r2_maf_"$FileExtension".txt
...
### PCA
echo "GCTA: PCA (imputation set)"
gcta64 \
   --grm-bin $ACC/012/predGRM_"$FileExtension" \
   --pca 2 \
   --threads $THREADS \
   --out $ACC/012/predGRM_"$FileExtension" > /dev/null 2>&1
wait
...
rm $ACC/012/predGRM_"$FileExtension".log > /dev/null 2>&1 
...
awk '{print $1":"$4}' $PHASE/plink_"$FileExtension".bim > $PHASE/pos_"$FileExtension".txt
wait
